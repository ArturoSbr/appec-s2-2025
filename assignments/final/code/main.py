...
ks = pd.get_dummies(
    data=es.loc[es['treat'].eq(1), ['k']],  # Filter out never-treated units
    columns=['k'],
    dtype=int
)
...
for g in sorted(G):
    for t in sorted(T):
        ...
        mask_control = (
            df['year'].eq(t)  # Year equals t
            & (
                # Condition 1: Never-treated units
                df['treat'].eq(0)  # treat column
                # Condition 2: Not-yet treated units
                | (
                    df['first_treat'].gt(t)  # Not-yet treated
                    & df['first_treat'].ne(g)  # Not cohort g
                )
            )
        )
        ...
        mask_treatment = (
            ...
            & df['first_treat'].eq(g)  # 3. Treatment cohort
        )
...
# 3.2 Make never-treated units have np.nan instead of 0 in `first_treat`
