...
# Caliper: max allowable distance between propensity scores
THR = 0.05

# Iterate through columns (each column i represents the i-th treated worker)
for i in range(dist.shape[1]):

    # Distances from every control to the i-th treated unit
    a = dist[:, i]
    ...
    # Apply the caliper: only keep matches within the threshold
    if val <= THR:
        ...
        # Setting the whole row to infinity guarantees this control unit
        # will never be the argmin in future iterations.
        dist[idx, :] = np.inf
